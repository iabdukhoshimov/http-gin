...
func InsertMovie(db *sql.DB, movie models.Movie) (string, error) {
	...
	// exec is used at create, delete and update

	// query.row is used at select one row or one column or get all rows
	_, err := db.Exec(query, // exec is the same at sql.db and pgx.pool
		id,                // uuid generate boladi
		movie.Title,       // userdan keladi
		movie.Duration,    // userdan keladi
		movie.Description, // userdan keladi
	)
